...
${(analysis.nextGoals||[]).length ? `<div style="background:#0d1a2d;border:1px solid #4F8EF733;border-radius:10px;padding:16px;margin-bottom:12px"><div style="color:#4F8EF7;font-size:11px;font-weight:600;letter-spacing:1px;margin-bottom:8px">📋 RECOMMENDED NEXT</div>${(analysis.nextGoals||[]).map(g=>`<div style="color:#ccccdd;font-size:13px;padding:3px 0">• ${g}</div>`).join('')}</div>` : ''}
${(analysis.topicsCovered||[]).length ? `<div style="color:#8888aa;font-size:12px;margin-bottom:12px">Topics covered: ${(analysis.topicsCovered||[]).join(', ')}</div>` : ''}
