...
export default function MusicForAllPage() {
  ...
  const sendMessage = async (text) => {
    ...
    setMessages(prev);
    try {
      const res = await fetch("/api/chat",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({model:"claude-3-haiku-20240307",max_tokens:1500,system:buildSystemPrompt(profile),messages:prev.map(m=>({role:m.role,content:m.content}))})});
