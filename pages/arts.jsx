  ...
  async function showDrawing() {
    ...
    try {
      const res = await fetch('/api/anthropic', {
        ...
        body: JSON.stringify({
          model: /* PERMANENT: Haiku 3 only. Never change without Khurram's approval. */ 'claude-3-haiku-20240307', max_tokens: 1200,
  ...
  const sendMessage = async (text) => {
    ...
    try {
      if (hasImage) {
        ...
        const res = await fetch('/api/anthropic', {
          method: 'POST', headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ model: /* PERMANENT: Haiku 3 only. Never change without Khurram's approval. */ 'claude-3-haiku-20240307', max_tokens: 1200, system: buildPrompt(stage, topic) + '\n\nThe student is sharing artwork or an image. Give warm, specific, encouraging feedback. Notice techniques, colours, composition, effort.', messages: apiMessages })
        });
        const data = await res.json();
