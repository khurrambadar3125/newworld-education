...
/**
 * /news — Education news and insights
...
export default function NewsPage() {
  ...
  const [articles, setArticles] = useState([...SEED_ARTICLES, ...SEO_ARTICLES]);
  ...
  useEffect(() => {
    fetch('/api/news-articles').then(r => r.json()).then(data => {
      if (data.articles?.length) setArticles([...data.articles, ...SEED_ARTICLES, ...SEO_ARTICLES]);
    }).catch(() => {});
  }, []);
      ...
      <div style={{ minHeight: "100vh", background: "#080C18", fontFamily: "'Sora',-apple-system,sans-serif", color: "#fff" }}>
        <Head>
          ...
          <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify({
            ...
            "author": { "@type": "Person", "name": selected.author || "NewWorld Education" },
            "publisher": {
              "@type": "Organization",
              "name": "NewWorld Education",
              "url": "https://www.newworld.education",
              "logo": { "@type": "ImageObject", "url": "https://www.newworld.education/icons/icon-512.png", "width": 512, "height": 512 },
            },
            "mainEntityOfPage": { "@type": "WebPage", "@id": `https://www.newworld.education/news?article=${selected.id}` },
            "image": "https://www.newworld.education/icons/icon-512.png",
            "articleSection": selected.category || "Education",
            "inLanguage": "en",
          })}} />
        </Head>
        ...
        <article style={{ maxWidth: 680, margin: "0 auto", padding: isMobile ? "28px 16px" : "48px 24px" }}>
          ...
          <div style={{ marginTop: 32, padding: 16, background: "rgba(255,255,255,0.02)", borderRadius: 10, border: "1px solid rgba(255,255,255,0.04)" }}>
            <p style={{ fontSize: 11, color: "rgba(255,255,255,0.3)", lineHeight: 1.7, margin: 0 }}>
              <strong>Disclaimer:</strong> This article is generated by AI for educational and informational purposes only. It does not constitute professional academic, medical, or legal advice. While we strive for accuracy, content may contain errors or outdated information. Readers should verify critical information with qualified professionals or official sources. NewWorld Education is not liable for any decisions made based on this content. Views expressed are generated perspectives and do not necessarily reflect the opinions of any examination body, educational institution, or regulatory authority referenced herein.
            </p>
          </div>
        </article>
        <LegalFooter />
    ...
    <div style={{ minHeight: "100vh", background: "#080C18", fontFamily: "'Sora',-apple-system,sans-serif", color: "#fff" }}>
      <Head>
        ...
        <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify({
          ...
          "name": "NewWorld Education News",
          "url": "https://www.newworld.education/news",
          "publisher": {
            "@type": "Organization",
            "name": "NewWorld Education",
            "url": "https://www.newworld.education",
            "logo": { "@type": "ImageObject", "url": "https://www.newworld.education/icons/icon-512.png" },
          },
          "mainEntity": {
            "@type": "ItemList",
            "itemListElement": articles.slice(0, 20).map((a, i) => ({
              "@type": "ListItem", "position": i + 1,
              "url": `https://www.newworld.education/news?article=${a.id}`,
              "name": a.title,
            })),
          },
        })}} />
