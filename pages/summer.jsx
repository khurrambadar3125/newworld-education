  ...
  const sendMessage = async (text) => {
    ...
    try {
      ...
      const res = await fetch("/api/chat", {
        ...
        body: JSON.stringify({ model: /* PERMANENT: Haiku 3 only */ "claude-haiku-4-5-20251001", max_tokens: group?.id === 'scholars' ? 1200 : 800, system, messages: prev.map(m=>({role:m.role,content:m.content})) }),
      });
