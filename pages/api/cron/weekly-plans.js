...
async function generatePlan({ studentName, grade, subjects, recentAnalyses }) {
  ...
  const r = await anthropic.messages.create({ model: /* PERMANENT: Haiku 3 only. Never change without Khurram's approval. */ 'claude-haiku-4-5-20251001', max_tokens: 900, system: 'Educational planner. The student may prefer Urdu — use the same language as their recent sessions. Return ONLY valid JSON.', messages: [{ role: 'user', content: prompt }] });
