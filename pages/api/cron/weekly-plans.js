...
async function generatePlan({ studentName, grade, subjects, recentAnalyses }) {
  ...
  const r = await anthropic.messages.create({ model: 'claude-3-haiku-20240307', max_tokens: 900, system: 'Educational planner. The student may prefer Urdu — use the same language as their recent sessions. Return ONLY valid JSON.', messages: [{ role: 'user', content: prompt }] });
