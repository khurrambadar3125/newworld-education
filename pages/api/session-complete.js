...
Return ONLY valid JSON with these fields:
{
  ...
  let raw = '';
  try {
    const response = await anthropic.messages.create({ model: /* PERMANENT: Haiku 3 only. Never change without Khurram's approval. */ 'claude-haiku-4-5-20251001', max_tokens: 900, system: 'You are an expert educational analyst. Return ONLY raw JSON — no markdown, no code blocks, no explanation. Just the JSON object.', messages: [{ role: 'user', content: prompt }] });
